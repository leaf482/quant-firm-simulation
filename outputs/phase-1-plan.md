# Phase 1 proposal — pending review
...
1. **Approve scope and bootstrap:** after user review, create the Go module/CLI in the existing Git repository, validate paper-only configuration, and document the single-instrument constraints. Acceptance: unsupported execution modes and invalid configuration fail before processing data.
...
3. **Build journal and recovery foundation:** implement single-writer ownership, durable append, schema/integrity checks, fingerprint validation, and restore primitives. Acceptance: interrupted writes fail clearly; complete records restore in order; storage failures halt processing.
4. **Build replay and engine:** validate CSV, establish deterministic ordering and an injected clock, commit input progress, and implement shutdown. Acceptance: duplicate/conflicting/out-of-order records behave as specified and a fixture replays deterministically.
5. **Build portfolio accounting:** apply fills idempotently, calculate average cost and realized/unrealized PnL, and mark quotes. Acceptance: hand-calculated buy/sell/fee examples reconcile; duplicate fills leave balances unchanged.
6. **Build risk and reservations:** validate limits, holdings, cash, fees, and freshness with pending orders included. Acceptance: multiple outstanding intents cannot reuse reserved resources; rejection changes no holdings.
7. **Build OMS:** persist intent-to-order identity, submission state, terminal outcomes, and reservation release. Acceptance: duplicate intents/submissions and terminal events retain one order and release capacity once; no path bypasses risk.
8. **Build paper broker:** implement next-quote full fills, affordability recheck, deterministic fill IDs, and EOF expiration. Acceptance: no same-quote fills, no duplicate fills, no negative cash/short positions, and explicit results for price gaps.
9. **Wire the fixture strategy and observability:** integrate the entire flow, structured logs, and run summary. Acceptance: one local command produces a traceable quote-to-fill-to-PnL run with manually verifiable expected balances.
10. **Exercise recovery and document operation:** test restart around acceptance/fill commits, duplicate delivery, journal failure, and graceful shutdown; document replay/resume/new-run commands and limitations. Acceptance: uninterrupted and resumed runs produce identical orders, fills, balances, and domain outcomes.
